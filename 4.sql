create table STUDENT(usn varchar(20) primary key, sname varchar(20), address varchar(20), phone
bigint, gender varchar(1));

create table SEMSEC(ssid varchar(5) primary key, sem integer, sec varchar(1));

create table SUBJECT(subcode varchar(10) primary key, title varchar(5), sem integer, credits integer);

create table CLASS(usn varchar(20), foreign key(usn) references STUDENT(usn) on delete cascade,
ssid varchar(5), foreign key(ssid) references SEMSEC(ssid) on delete cascade, primary key(usn,ssid));

create table IAMARKS(usn varchar(20), foreign key(usn) references STUDENT(usn) on delete
cascade, subcode varchar(10), foreign key(subcode) references SUBJECT(subcode) on delete
cascade, ssid varchar(5), foreign key(ssid) references SEMSEC(ssid) on delete cascade, test1 integer,
test2 integer, test3 integer, finalia integer, primary key(usn,subcode,ssid));
...
-- Queries:
select s.*,ss.sem, ss.sec from STUDENT s, SEMSEC ss, CLASS c where s.usn = c.usn and ss.ssid =
c.ssid and ss.sem = 4 and ss.sec='C';

select ss.sem, ss.sec, s.gender, count(s.gender) as count from STUDENT s, SEMSEC ss, CLASS c
where s.usn = c.usn and ss.ssid = c.ssid group by ss.sem, ss.sec, s.gender order by ss.sem;

create view STU_TEST_MARKS as select test1,subcode from IAMARKS where usn='1RN13CS091';
select * from STU_TEST_MARKS;
...
'WEAK'
END) AS CAT
from student s, iamarks ia, subject sub, semsec ss where s.usn = ia.usn and ss.ssid = ia.ssid and
